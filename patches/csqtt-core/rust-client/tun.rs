...
mod linux {
    ...
    impl FdReceiver {
        pub fn bind(name: &str) -> Result<Self> {
            ...
            let rc = unsafe {
                libc::bind(fd.as_raw_fd(), (&addr as *const libc::sockaddr_un).cast::<libc::sockaddr>(), addr_len)
            };
                ...
                loop {
                    match accept_nonblocking(self.listener.get_ref().as_raw_fd()) {
                        ...
                        Err(error) if error.kind() == io::ErrorKind::WouldBlock => {
                            guard.clear_ready(); break;
                        }
                        Err(error) => {
                            guard.clear_ready(); return Err(error.into());
                        }
                    }
                }
            }
    ...
    fn make_abstract_addr(name: &str) -> Result<libc::sockaddr_un> {
        ...
        if bytes.len() + 1 > UNIX_PATH_MAX { bail!("TUN UDS name is too long: {} bytes", bytes.len()); }
    ...
    async fn recv_tun_fd(fd: OwnedFd, cancel: &CancellationToken) -> Result<File> {
        ...
        // Ancillary data must be naturally aligned for cmsghdr/CMSG_DATA.
        ...
        loop {
            ...
            let mut cursor = control.as_ptr();
            let end = unsafe { control.as_ptr().add(msg.msg_controllen as usize) };
            while (cursor as usize) + size_of::<libc::cmsghdr>() <= end as usize {
                let header = unsafe { &*(cursor.cast::<libc::cmsghdr>()) };
                if header.cmsg_len < size_of::<libc::cmsghdr>() as libc::size_t { break; }
