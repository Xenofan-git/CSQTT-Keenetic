...
mod linux {
    ...
    impl FdReceiver {
        pub fn bind(name: &str) -> Result<Self> {
            ...
            let rc = unsafe { libc::bind(fd.as_raw_fd(), (&addr as *const libc::sockaddr_un).cast::<libc::sockaddr>(), abstract_addr_len(name.len())) };
        ...
        pub async fn receive(&self, cancel: &CancellationToken) -> Result<File> {
            loop {
                let mut guard = tokio::select! { _ = cancel.cancelled() => bail!("TUN FD receiver cancelled"), ready = self.listener.readable() => ready?, };
                loop {
    ...
    fn make_abstract_addr(name: &str) -> Result<libc::sockaddr_un> {
        ...
        for (i, byte) in name.as_bytes().iter().enumerate() { addr.sun_path[i + 1] = *byte as libc::c_char; }
        Ok(addr)
    }
    ...
    async fn send_tun_ack(socket: &AsyncFd<OwnedFd>, cancel: &CancellationToken) -> Result<()> {
        let ack = [1u8];
        loop {
            let mut guard = tokio::select! { _ = cancel.cancelled() => bail!("TUN FD ACK cancelled"), ready = socket.writable() => ready?, };
    ...
    async fn recv_tun_fd(fd: OwnedFd, cancel: &CancellationToken) -> Result<File> {
        ...
        loop {
            let mut guard = tokio::select! { _ = cancel.cancelled() => bail!("TUN FD transfer cancelled"), ready = socket.readable() => ready?, };
            let mut iov = libc::iovec { iov_base: payload.as_mut_ptr().cast(), iov_len: 1 };
            let mut msg: libc::msghdr = unsafe { zeroed() };
            msg.msg_iov = &mut iov; msg.msg_iovlen = 1; msg.msg_control = control.as_mut_ptr().cast(); msg.msg_controllen = std::mem::size_of_val(&control) as libc::socklen_t;
            let n = unsafe { libc::recvmsg(socket.get_ref().as_raw_fd(), &mut msg, 0) };
            if n < 0 { let error = io::Error::last_os_error(); guard.clear_ready(); if error.kind() == io::ErrorKind::WouldBlock { continue; } return Err(error.into()); }
            ...
            while (cursor as usize) + size_of::<libc::cmsghdr>() <= end as usize {
                ...
                if header.cmsg_level == libc::SOL_SOCKET && header.cmsg_type == libc::SCM_RIGHTS {
                    ...
                    if (header.cmsg_len as usize).saturating_sub(base) < size_of::<RawFd>() { guard.clear_ready(); return Err(anyhow::anyhow!("SCM_RIGHTS contains no file descriptor")); }
                    let received_fd = unsafe { *libc::CMSG_DATA(header).cast::<RawFd>() };
...
mod tests {
    #[cfg(target_os = "linux")]
    #[test]
    fn abstract_addr_length_is_correct() { assert_eq!(super::linux::abstract_addr_len(0), 3); assert_eq!(super::linux::abstract_addr_len(14), 17); }
}
