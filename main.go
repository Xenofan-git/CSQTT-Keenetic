...
func main() {
    ...
    if err != nil { stdin.Close(); log.Fatalf("stdout pipe: %v", err) }
    if err := cmd.Start(); err != nil { stdin.Close(); log.Fatalf("start client: %v", err) }
    ...
    if !sent { _ = cmd.Process.Kill(); stdin.Close(); <-clientDone; log.Fatal("could not pass TUN FD to client") }
    clientIP, err := waitForTUNCONF(lines, 30*time.Second)
    if err != nil { _ = cmd.Process.Kill(); stdin.Close(); log.Fatalf("TUNCONF: %v", err) }
    log.Printf("server assigned TUN IP: %s", clientIP)
    if err := configureTUN(clientIP); err != nil { _ = cmd.Process.Kill(); stdin.Close(); log.Fatalf("configure TUN: %v", err) }
