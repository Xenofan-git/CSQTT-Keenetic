...
function updateVKBookmarklet(){
  ...
  if(!out) return;
  const code="javascript:(()=>{try{const p=new URLSearchParams(location.hash.slice(1)),t=p.get('access_token')||'',s=p.get('state')||'';if(!t)throw Error('VK не вернул access_token');if(!s)throw Error('нет state CSQTT');let d=s.replace(/-/g,'+').replace(/_/g,'/');while(d.length%4)d+='=';const raw=new TextDecoder().decode(Uint8Array.from(atob(d),c=>c.charCodeAt(0))),i=raw.indexOf('|'),cb=raw.slice(0,i),panel=cb.replace(/\\/api\\/vk\\/token$/,'')+'#vk';if(!/^https?:\\/\\/[^|]+\\/api\\/vk\\/token$/i.test(cb))throw Error('неверный CSQTT callback');document.documentElement.innerHTML='<head><meta name=\\\"viewport\\\" content=\\\"width=device-width,initial-scale=1\\\"><title>CSQTT VK</title><style>*{box-sizing:border-box}body{margin:0;min-height:100vh;font-family:system-ui,-apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;background:linear-gradient(180deg,#fff,#f5f7fb);color:#111;padding:24px}.box{max-width:560px;margin:8vh auto 0;background:#fff;border:1px solid #e3e7ee;border-radius:28px;padding:30px 24px;box-shadow:0 14px 42px rgba(0,0,0,.12);text-align:center}.logo{width:72px;height:72px;margin:0 auto 18px;border-radius:22px;background:linear-gradient(145deg,#e8f0fe,#fff);display:flex;align-items:center;justify-content:center;color:#1683df;font-size:38px;font-weight:800}.title{font-size:24px;font-weight:750;margin:0 0 12px}.text{font-size:17px;line-height:1.5;color:#5f6067;margin:0 0 24px}.btn{display:block;width:100%;border:0;border-radius:17px;padding:15px 18px;background:linear-gradient(135deg,#1976d2,#1565d8);color:#fff;font-size:16px;font-weight:750}</style></head><body><main class=box><div class=logo>C</div><h1 class=title>VK авторизация</h1><p class=text>Скопируйте ссылку этой страницы из адресной строки, затем нажмите кнопку ниже, чтобы вернуться в панель CSQTT и вставить эту ссылку в поле авторизации.</p><button class=btn id=back>Раздел авторизации</button></main></body>';document.getElementById('back').onclick=()=>location.href=panel}catch(e){document.documentElement.innerHTML='<body style=\\\"font-family:system-ui;padding:30px\\\"><h2>Ошибка VK</h2><p>'+String(e.message).replace(/[<>]/g,'')+'</p></body>'}})();";
  out.value=code;
}
